...
export class ContactsService {
  ...
  constructor(private http: Http) {
    this.initContacts();
    this.currentContact = new Contact(null, "Jordan Aquino", "jaquinof@pucp.pe", "123-134-1245",
      "",null);
  }

  initContacts(){
    return this.http.get('https://contactapp-c84d3.firebaseio.com/contacts.json')
      .map((response: Response) => response.json())
      .subscribe(
        (data: Contact[]) => {
          this.contacts = data;
          //this.currentContact = this.getContact("18");
  ...
  getContact(idx:string){
    ...
    // if(idx < 0 || idx >=this.contacts.length){
    //   throw "Array index out of bounds.";
    // }
    //return this.contacts[idx];
    return this.contacts.find((contact: Contact) => contact.name === idx);
  ...
  compareNames(contactA: Contact, contactB: Contact) {
    ...
    if (contactA.description < contactB.description)
      return -1;
    if (contactA.description > contactB.description)
      return 1;
    return 0;
  ...
  addContact(contact: Contact){
    if(!contact)
      return;
    contact.name = this.getAvailableContactId();
  ...
  storeContacts() {
    ...
    return this.http.put('https://contactapp-c84d3.firebaseio.com/contacts.json',
      body, {headers: headers}).toPromise();
  }

  getAvailableContactId() : string {
    let tempArray = this.contacts.sort(this.compareId);
    let i = 0;
  ...
  compareId(contactA: Contact, contactB: Contact) {
